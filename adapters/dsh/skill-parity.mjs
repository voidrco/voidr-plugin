...
`)
    ...
    content = replaceSection(content, 'Track the execution until a terminal state', '### When to offer cancelling',
      'Track the execution with assistant_workspace_validation_status(sessionId, executionId). QUEUED and RUNNING are active work: wait at least 30 seconds between polls and continue in the same turn until a terminal verdict. Never end the turn by asking the user to type a wake word such as “verifica”, “continue” or “check again”, and never present a non-terminal status as consolidated. If the host turn deadline makes further observation impossible, call ask_user_question with id automate-validation-running and offer “Continuar acompanhando (Recomendado)” or “Encerrar acompanhamento”; a plain chat request to resume is forbidden. Report the execution link with the terminal verdict.\n\n')
...
- QUEUED and RUNNING validation is active work. Keep polling in the same turn until a terminal verdict; never hand polling back to the user through a plain-text wake word. If the host turn deadline prevents another poll, use ask_user_question with id automate-validation-running and explicit continue or stop choices.
