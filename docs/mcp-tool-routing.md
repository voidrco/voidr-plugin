...
| `voidr_workspace_bootstrap_test_repository` | `voidr-develop-tests` | Initialize the test-project skeleton in a confirmed empty destination, or in an origin-matching checkout that lacks test-project files. Never clones — cloning belongs to `voidr_workspace_prepare_test_repository`, which is also the only path for a plan whose repository Voidr already provisioned: this tool refuses that plan instead of writing a skeleton the preparation gate would reject. Detects an existing checkout by origin and returns `reusedExistingCheckout`. |
