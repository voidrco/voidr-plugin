...
| `test_plans_provision_repository` | `voidr-test-plan`, `voidr-feature-test`, `voidr-implement-tests` | Provision and link the repository of a plan that was created while provisioning was unavailable, so automation can start later without creating a second plan. Idempotent: a plan that already has a repository returns the stored result. |
